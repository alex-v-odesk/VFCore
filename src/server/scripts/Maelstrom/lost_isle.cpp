        ...
        void UpdateAI(uint32 diff)
        {
            if (!isEventInProgress)
            {
                if (mui_talk <= diff)
                {
                    ...
                    {
                    case 0 :
                        me->MonsterSay(SAY_D_A, LANG_UNIVERSAL, player);
                        break;
                    case 1 :
                        me->MonsterSay(SAY_D_B, LANG_UNIVERSAL, player);
                        break;
                    case 2 :
                        if (Creature *c = me->FindNearestCreature(GIZMO, 10))
                            c->MonsterSay(SAY_D_C, LANG_UNIVERSAL, player);
                        break;
                    case 3 :
                        me->MonsterSay(SAY_D_D, LANG_UNIVERSAL, player);
            ...
            if (mui_talk2 <= diff)
            {
                ...
                {
                case 0 :
                    me->MonsterSay(SAY_D_E, LANG_UNIVERSAL, player);
                    me->CastSpell(player, 54732, true);
                    player->RemoveAurasDueToSpell(SPELL_DEAD_STILL);
					player->SetPhaseMask(1, true);
                    break;
                case 1 :
                    me->MonsterSay(SAY_D_F, LANG_UNIVERSAL, player);
                    break;
                case 2 :
                    me->MonsterSay(SAY_D_G, LANG_UNIVERSAL, player);
                    break;
                case 3 :
                    me->CastSpell(player, 54732, true);
                    me->MonsterSay(SAY_D_H, LANG_UNIVERSAL, player);
                    mui_talk2 = 2000;
                    break;
                case 4 :
    ...
    struct npc_mineur_gobAI : public npc_escortAI
    {
        ...
        void WaypointReached(uint32 i)
        {
            ...
            case 18:
                ...
                {
                    me->MonsterSay("It will suffice for now. I can go it alone. Thank you for escorting me, $ N.", LANG_UNIVERSAL, player);
    ...
    struct npc_crack_isle_gobAI : public ScriptedAI
    {
        ...
        void Reset ()
        {
            ...
            if (Player *player =  Unit::GetPlayer(*me, playerGUID))
            {
                ...
                me->MonsterSay(QUEST_RESET_CRACK, LANG_UNIVERSAL, player);
            }
            else
                me->DespawnOrUnsummon();
            ui_findPlayer = 1000;
        ...
        void UpdateAI(uint32 diff)
        {
            if (!start)
            {
                if (ui_findPlayer <= diff)
                {
                    if (Creature *zone = me->FindNearestCreature(38450, 3))
                    {
                        if (Player *player =  Unit::GetPlayer(*me, playerGUID))
                        {
                            me->MonsterSay(CRACK_PROVOC, LANG_UNIVERSAL, player);
            ...
            else if (event)
            {
                if (mui_event <= diff)
                {
                    ...
                    case 0 :
                        if (Player *player =  Unit::GetPlayer(*me, playerGUID))
                            naga->MonsterYell(NAGA_SAY_A, LANG_UNIVERSAL, player);
                        break;
                    case 1 :
                        if (Player *player =  Unit::GetPlayer(*me, playerGUID))
                            naga->MonsterYell(NAGA_SAY_B, LANG_UNIVERSAL, player);
                        break;
                    case 2 :
                        if (Player *player =  Unit::GetPlayer(*me, playerGUID))
                            naga->MonsterYell(NAGA_SAY_C, LANG_UNIVERSAL, player);
                        break;
                    case 3 :
                        if (Player *player =  Unit::GetPlayer(*me, playerGUID))
                            naga->MonsterYell(NAGA_SAY_D, LANG_UNIVERSAL, player);
                        break;
                    default :
                        if (Player *player =  Unit::GetPlayer(*me, playerGUID))
                            me->MonsterYell(CRACK_EVADE, LANG_UNIVERSAL, player);
                        mui_event = 2000;
                        event = false;
                        combats = true;
                        break;
    ...
    struct npc_avion_gobAI : public npc_escortAI
    {
        ...
        void WaypointReached(uint32 i)
        {
            ...
            switch(i)
            {
                case 1:
                    if (Player *player = me->FindNearestPlayer(10))
                        chipie->MonsterSay("Hang on $ N! We need makes a clearance here, NOW!", LANG_UNIVERSAL, player);
                    break;
                case 4 :
